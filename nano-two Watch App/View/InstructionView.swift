    ...
    var body: some View {
        VStack {
            ...
            if isWithinRange(userLatitude: userLatitude, userLongitude: userLongitude, location: location) {
                NavigationLink(destination: GameView(godName: location.god, progressManager: progressManager, locationIndex: locationIndex)) {
                    Text("Start Puzzle")
                        .font(.title3)
                        .padding()
...
#Preview {
    InstructionView(location: Location(locationName: "San Francisco Museum", latitude: 37.78584535799374, longitude: -122.40101861564061, god: "OSIRIS"), userLatitude: 37.78584535799374, userLongitude: -122.40101861564061, progressManager: ProgressManager(totalLocations: 8), locationIndex: 0)
}
